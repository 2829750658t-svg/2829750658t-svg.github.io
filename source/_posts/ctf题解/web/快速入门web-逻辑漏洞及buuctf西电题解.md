---
title: '快速入门web-逻辑漏洞及buuctf西电题解'
categories:
  - 
tags: []
abbrlink: 708b800e
date: 2026-03-07 14:29:29
---
...
![image-20260101182325159](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101182325159.png)
...
![image-20260101182556727](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101182556727.png)

千辛万苦改完了，但是他说你不是他的学生

![image-20260101182624105](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101182624105.png)
...
![image-20260101194019184](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101194019184.png)
...
![image-20260101193118587](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101193118587.png)
...
![image-20260101185924710](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101185924710.png)
...
![image-20260101190705612](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101190705612.png)
...
![image-20260101190756691](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101190756691.png)
...
![image-20260101191116410](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101191116410.png)



![image-20260101192942012](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260101192942012.png)
...
![image-20251220205145783](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20251220205145783.png)
...
![image-20251220205251772](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20251220205251772.png)
...
![image-20251227163010489](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20251227163010489.png)
...
![image-20251227201231091](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20251227201231091.png)
...
md5和sha1都不能处理数组，所以返回值都是null，

但因为eval($this->syc);所以不能用数组绕过 -> 考虑原生类
...
$str = "?><?=include~" . urldecode("%D0%99%93%9E%98") . "?>";	//短标签

$c = new SYCLOVER();

$c->syc = new Error($str, 1);$c->lover = new Error($str, 2);	//注意行号和code
...
为了执行代码我们需要`eval()` 或 `system()` 这种明显的入口。

而想要向里面塞指令必须要满足条件：这道题需要进入魔法函数，然后绕过md5和过滤，才能执行eval
...
把不同的魔法函数可以在对应的不同情况下被触发，比如error类里面__tostring函数触发点：当error类被当成字符串时

所以，md5在处理syc的时候会把他当成字符串，从而触发了error类，那么就执行了这个魔法函数，得到**字符串：** `"Error: [你的代码] in [文件路径]:[行号]"`
...
```
Error：用于PHP7、8，开启报错。（所有php内部错误类的基类）
可以在echo时触发__tostring,来构造xss
...
Exception 和 Error ，他们有一个 __toString 方法，当类被当做字符串处理时，就会调用这个函数
...
原生列里面有些信息是受保护的，在被序列化时会加上空字节`\0`，如果没有被url编码就会被认为是结束符



###### 4.不能用(),那怎么运行eval？eval执行逻辑？

4.1用include读取，include可以不用()

`include` 后面跟着的是一个表达式，比如include~(...)
...
`eval($code)` 它的作用是将字符串当作 PHP 代码执行。

**常用函数**：`system()`、`passthru()`、`exec()`、`shell_exec()`、反引号 ```。

**Payload 示例**： `eval("system('cat /flag');");`

由于error的魔法返回前面有一些信息，现在我们还不清楚那一堆杂质是什么模式，所以不管他是html模式还是php代码解析模式，

先给他闭合，用?>

再用<?打开一个新的语句，并在里面写入include代码执行，
...
![image-20260127211425730](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260127211425730.png)

以为是cookie参数改成跟user，monster，admin相关的东西，但是抓包没有发现cookie啊

但是题目说是要像admin一样去登录，关键这万能密码也没有用啊
...
我来了点灵感，按照get传参action=login

发现这个页面的返回页面居然有cookie呢

![image-20260127211803876](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260127211803876.png)

诶哟，直接拿过来传参

![image-20260127211857722](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260127211857722.png)
...
$obj->source = $obj;
$obj->str = new Test;
$obj->str->p = new Modifier;
$obj->str->p->var = 'php://filter/read=convert.base64-encode/resource=flag.php';//但是这里的var是受保护的，我们不能直接在外面赋值，需要在下面的类中直接赋值
修改如下：
class Modifier {
...
当你的source指向了一个类，那么我们的preg_match就没办法处理了，他只会处理字符串

那怎么办呢？他回去看看类下面有什么函数可以返回字符串给他的，这时候我们已经开始了tostring执行我们的pop链了

但是：为什么不出发__construct？

反序列化不会触发 __construct，因为反序列化只是恢复对象并没有创建类

那我们在写new代码的时候不已经触发了么？

不太一样

——

**本地的 `new`**：是为了产生一个可以被序列化的**素材**。

**服务器的 **：并没有看到new，对他来说，只是看到一个@unserialize($_GET['pop']);
...
md5解码后发现是一条购物记录，要付费才能破解给你看

直接跳过

换了另一个网站解码，得到了 `ARandomString`，输入验证码后也被识破了

那么直接访问flag.php呢，发现

![image-20260128122110830](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260128122110830.png)

当然也一无所获，但仔细想想，

"我已经把购买者的IP保存了，显然你没有购买"这句话已经告诉我们和购买者的ip有关了。

->

在 PHP 中，开发者通常会用以下代码获取 IP：

- `$_SERVER['REMOTE_ADDR']`：这是最安全的，因为它记录的是 TCP 连接的真实 IP（难以伪造）。
- **`$_SERVER['HTTP_X_FORWARDED_FOR']`**：这是最**不安全**的，因为它从 HTTP 头部读取数据。

**为什么开发者要用不安全的 header？** 因为当网站使用了 CDN（如 Cloudflare）或反向代理（如 Nginx）时，`REMOTE_ADDR` 拿到的永远是代理服务器的 IP。为了获取真正的用户 IP，开发者被迫去读取 `X-Forwarded-For`。这就给了我们伪造的机会。

->

为什么你不是购买者，因为后端服务器的判断

那么怎么让他判断你是购买者呢？通过http请求头修改你的ip就好了

->

或许抓包看看能不能塞个xff进去

![image-20260128122306613](https://cdn.jsdelivr.net/gh/2829750658t-svg/Blog-images@main/image-20260128122306613.png)
